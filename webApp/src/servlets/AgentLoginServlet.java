...
@WebServlet(name = "AgentLoginServlet", urlPatterns = {"/agentLogin"})
public class AgentLoginServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (usernameFromSession == null) { //user is not logged in yet
            ...
            String teamFromParameter = request.getParameter(TEAM);
            String threadsFromParameter = request.getParameter(THREADS);
            String tasksPerPullFromParameter = request.getParameter(TASKSPERPULL);
            ...
            if (usernameFromParameter == null || usernameFromParameter.isEmpty()) {
                ...
                synchronized (this) {
                    if (userManager.isUserExists(usernameFromParameter)) {
                        ...
                        Ally ally = userManager.getAlly(teamFromParameter);
                        Agent agent = new Agent(usernameFromParameter, teamFromParameter, ally.getBattlefieldAssignedTo(), Integer.parseInt(threadsFromParameter),
                                Integer.parseInt(tasksPerPullFromParameter));
                        ally.addAgent(usernameFromParameter, agent);
                        userManager.addEntity(usernameFromParameter, agent, UserType.AGENT);

                        //set the username in a session so it will be available on each request
                        //the true parameter means that if a session object does not exists yet
                        //create a new one
                        request.getSession(true).setAttribute(Constants.USERNAME, usernameFromParameter);
                        request.getSession(true).setAttribute(Constants.USERTYPE, UserType.AGENT);
                        //redirect the request to the chat room - in order to actually change the URL
                        System.out.println("On login, request URI is: " + request.getRequestURI());
                        response.setStatus(HttpServletResponse.SC_OK);
                        response.getWriter().println("Login succeeded");
                    }
                }
            }
        } else {
